...
ALTER TABLE Users ADD WalletBalance DECIMAL(18,2) DEFAULT 0;
ALTER TABLE PostpaidBills 
ADD CONSTRAINT FK_Postpaid_Transaction 
FOREIGN KEY (PaymentTransactionID) REFERENCES Transactions(TransactionID);

SELECT * FROM INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE
WHERE TABLE_NAME = 'PostpaidBills';

INSERT INTO PostpaidBills (MobileNumber, BillingCycle, TotalAmount, PaymentDueDate, IsPaid)
SELECT MobileNumber, GETDATE(), PostpaidMonthlyFee, DATEADD(DAY, 10, GETDATE()), 0
FROM Users
WHERE PostpaidMonthlyFee IS NOT NULL AND PostpaidMonthlyFee > 0
  AND MobileNumber NOT IN (
        SELECT MobileNumber FROM PostpaidBills
        WHERE MONTH(BillingCycle) = MONTH(GETDATE())
          AND YEAR(BillingCycle) = YEAR(GETDATE())
  );
